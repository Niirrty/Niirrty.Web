...
class Url
{
    ...
    *
    * @param string              $scheme
    * @param \Niirrty\Web\Domain $domain
    */
    ...
    *
    * @return \Niirrty\Web\Url|null Returns the real URL, or NULL.
    */
   public function extractUrlShortenerTarget() : ?Url
   {
      ...
      catch ( \Throwable $ex ) { unset( $ex ); return null; }
   ...
   public function checkForOpenRedirect( $urlForTestContents, $testContents, $useAsRegex = false )
   {
      ...
      if ( $handleHeaders && \count( $headers ) > 0 )
      {
         ...
         if ( isset( $headers[ 'refresh' ] ) && \Niirrty\strContains( $headers[ 'refresh' ], $urlForTestContents ) )
         {
   ...
   public function getPort() : int
   {
      ...
      {
         case UrlScheme::HTTP : return 80;
         case UrlScheme::SSL  : return 443;
         case UrlScheme::FTP  : return 21;
         default              : return 0;
      }
   ...
   public function getPath() : string
   {
      ...
      if ( ! \Niirrty\strStartsWith( $this->path, '/' ) )
      {
         return '/' . $this->path;
      }
    ...
    * @return \Niirrty\Web\Url|bool Returns the URL if parsing was successful, FALSE otherwise.
    */
   public static function Parse( ?string $urlString )
   {
      ...
      if ( isset( $objectData[ 'scheme' ] ) ) { $scheme = \strtolower( $objectData[ 'scheme' ] ); }
