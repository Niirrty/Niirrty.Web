...
class TopLevelDomain
{
   ...
   private static function parseForStates( TopLevelDomain $tld )
   {
      ...
      // FULLYQUALIFIED
      if ( \Niirrty\strEndsWith( $tld->value, '.' ) )
      {
      ...
      if ( \Niirrty\strContains( $tld->value, 'xn--' ) )
      {
         $tld->states[ 'LOCALIZED' ] = true;
         $tld->states[ 'KNOWN' ] = true;
      }
    ...
    * @param  \Niirrty\Web\TopLevelDomain $parsedTldOut   Returns the TopLevelDomain if the method returns true
   ...
   public static function TryParse(
      ?string $tld, ?TopLevelDomain &$parsedTldOut = null, bool $allowOnlyKnown = false, bool $convertUniCode = true ) : bool
   {

      if ( $convertUniCode ) { $tld = idnToASCII( $tld ); }
    ...
    * @param  bool                        $allowOnlyKnown Are only known main TLDs allowed to be a parsed as a TLD?
    * @param  bool                        $convertUniCode Convert unicode TLDs to Puny code? (Default = TRUE)
    * @return \Niirrty\Web\TopLevelDomain|bool   Returns the TopLevelDomain or FALSE on error.
    */
      ...
      bool $convertUniCode = false ) : bool
   {
      ...
      if ( $convertUniCode ) { $hostString = idnToASCII( $hostString ); }
    ...
    * @return \Niirrty\Web\TopLevelDomain|bool   Returns the TopLevelDomain or FALSE on error.
    */
   public static function ParseExtract( string &$hostString, bool $allowOnlyKnown = false, bool $convertUniCode = false)
   {
   ...
   public static function EndsWithValidTldString( string &$stringToCheck, bool $convertUniCode = false ) : bool
   {
      ...
      if ( $convertUniCode ) { $stringToCheck = idnToASCII( $stringToCheck ); }
